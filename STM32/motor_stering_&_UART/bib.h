...
//Dodalem include dla 'abs' ,bez tego nie działa
#include <stdlib.h>
...
void InitFunction(WHEEL_Type* FL,WHEEL_Type* FR,WHEEL_Type* BL,WHEEL_Type* BR)//funkcja incalizujaca 4 obiekty
{
    ...
    /*
	FL->ramp_data.FinalSpeed=-40;
	FR->ramp_data.FinalSpeed=40;
	BL->ramp_data.FinalSpeed=-40;
	BR->ramp_data.FinalSpeed=40;*/

	FL->ramp_data.RisingTime=0.6;
	FR->ramp_data.RisingTime=0.6;
	BL->ramp_data.RisingTime=0.6;
	BR->ramp_data.RisingTime=0.6;
	...
	//__HAL_TIM_SET_COMPARE(FL->timer_data._htim_PWM, FL->timer_data._channel_PWM,30);
	//__HAL_TIM_SET_COMPARE(FR->timer_data._htim_PWM, FR->timer_data._channel_PWM,30);
	//__HAL_TIM_SET_COMPARE(BL->timer_data._htim_PWM, BL->timer_data._channel_PWM,60);
	//__HAL_TIM_SET_COMPARE(BR->timer_data._htim_PWM, BR->timer_data._channel_PWM,60);
}
...
void VelocityCalculation(WHEEL_Type* FL,WHEEL_Type* FR,WHEEL_Type* BL,WHEEL_Type* BR,int speed) //ta funkcja oblicza jaką predkosc ma mieć kazde kolo
{
	if((FR->MOTOR_STATE==DRIVE)&&(FL->MOTOR_STATE==DRIVE)&&(BR->MOTOR_STATE==DRIVE)&&(BL->MOTOR_STATE==DRIVE))//sprawdzam czy kazde kolo jest w stanie drive
	{
		if((FR->MOVE_STATE==FL->MOVE_STATE)&&(BR->MOVE_STATE==BL->MOVE_STATE)&&(FL->MOVE_STATE==BR->MOVE_STATE))//sprawdzam czy kazde kolo jest w tym samym trybie ruchu
		{
			...
			case RIGHT_SIDE:
				...
				BR->ramp_data.FinalSpeed=speed;
				break;
			//case DIAGONAL_P:  //dopisac reszte kodu
			//	break;
			//case DIAGONAL_L:
			//	break;
			case AROUND:
...
//	float I=14.1;
//	float D=0.0;
	float P = 0.15;  // Mniejsza wartość proporcjonalna
	float I = 10.0; // Zmniejsz, jeśli widzisz windup
	float D = 0.05; // Wprowadź, jeśli występuje oscylacja
...
int ReadInfoFromUart(char znak,WHEEL_Type* FL,WHEEL_Type* FR,WHEEL_Type* BL,WHEEL_Type* BR)
{
	...
	switch(znak)                    //a teraz sprawdzam w którym dokładnie trybie ruchu jestem
	{
		case '0':
			...
			speed=0;
			break;
		case 'a':                     //gdy przyjdzie a to jazda do przodu
			...
			speed=13;	//15
			break;
		case 'b':					//gdy przyjdzie 0 to jazda do tylu
			FL->MOVE_STATE=FORWARD;
			FR->MOVE_STATE=FORWARD;
			BL->MOVE_STATE=FORWARD;
			BR->MOVE_STATE=FORWARD;
			speed=-15;
			break;
		case 'c':					//gdy przyjdzie 3 to jazda obrot w lewo
			...
			speed=-12;	//-5
			break;
		case 'd':                    //gdy przyjdzie 3 to jazda obrot w prawo
			FL->MOVE_STATE=AROUND;
			FR->MOVE_STATE=AROUND;
			BL->MOVE_STATE=AROUND;
			BR->MOVE_STATE=AROUND;
			speed=12;	//5
			break;
		case 'e':						//gdy przyjdzie 3 to jazda prosto w prawo
			...
			speed=12;	//15
			break;
		case 'f':						//gdy przyjdzie 3 to jazda prosto w lewo
			FL->MOVE_STATE=RIGHT_SIDE;
			FR->MOVE_STATE=RIGHT_SIDE;
			BL->MOVE_STATE=RIGHT_SIDE;
			BR->MOVE_STATE=RIGHT_SIDE;
